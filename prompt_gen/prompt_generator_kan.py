...
class PromptGenerator(nn.Module):
    def __init__(self, ckpt_pth='./checkpoints/sam2.1_hiera_base_plus.pt'):
        ...
        self.feature_reducer = FeatureReducer(256, 16)

        self.kan_model = KAN(
            layers_hidden=[16 * 4 * 4, 128, 64, 4],  # Adjust input dimension
            grid_size=5,
            spline_order=3,
            scale_noise=0.1,
            scale_base=1.0,
    ...
    def forward(self, x, update_grid=False):
        ...
        # Pass through KAN
        bbox_predictions = self.kan_model(normalized_features, update_grid=update_grid)
