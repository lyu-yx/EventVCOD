...
from sam2.modeling.sam2_base_prompt_generator_visionfeat_gather_chg_feats_fusion_and_evt_twoenc import NO_OBJ_SCORE, SAM2Base
...
class SAM2VideoPredictor(SAM2Base):
    ...
    def _get_event_feature(self, inference_state, frame_idx, batch_size):
        ...
        if backbone_out is None:
            ...
            backbone_out = self.forward_event(event)
            # Cache the most recent frame's feature (for repeated interactions with
            # a frame; we can use an LRU cache for more frames in the future).
