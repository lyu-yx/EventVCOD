...
class EmbeddingGenerator(nn.Module):
    def __init__(
        ...
        self.sparse_embedder = nn.Sequential(
            ...
            # First convolution: aggregates multi-scale features
            nn.Conv2d(mask_in_chans * 5, embed_dim, kernel_size=1),
            nn.BatchNorm2d(embed_dim),
            nn.ReLU(inplace=True),
            # Second convolution: further refines the feature representation
            nn.Conv2d(embed_dim, embed_dim, kernel_size=3, padding=1),
            nn.BatchNorm2d(embed_dim),
            nn.ReLU(inplace=True),
            # Change pooling to output two spatial tokens instead of one
