...
from sam2.modeling.sam2_base_adp0225_emb import NO_OBJ_SCORE, SAM2Base
...
class SAM2VideoPredictor(SAM2Base):
    ...
    def add_new_points_or_box(
        ...
        zero_mask = torch.zeros((1, 1, 64, 64)).to(inference_state["device"])
    ...
    def add_new_none_mask(
        ...
        """Add none mask to have promptless SAM2."""
        obj_idx = self._obj_id_to_idx(inference_state, obj_id)
        point_inputs_per_frame = inference_state["point_inputs_per_obj"][obj_idx]
        mask_inputs_per_frame = inference_state["mask_inputs_per_obj"][obj_idx]


        zero_mask = torch.zeros((self.image_size, self.image_size)).to(inference_state["device"])
        ...
        for i in range(self.num_frames_embedding):
            if frame_idx + i < video_len:
                (
                    ...
                    _feat_sizes,
                ) = self._get_image_feature(inference_state, frame_idx + i, batch_size)
