...
from sam2.modeling.sam2_base_adp0306_img_video_dense_update_embgen_nocurvideo import SAM2Base
...
class SAM2TrainVCODPromptGenerator(SAM2Base):
    def __init__(
        self,
        image_encoder,
        ...
        freeze_short_long_relation_attention=False,
        **kwargs,
    ):
        ...
        if freeze_short_long_relation_attention:
            for p in self.short_long_relation_attention.parameters():
                p.requires_grad = False
    ...
    def forward(self, input: BatchedVideoDatapoint):
        ...
        # for img input 
        # len backbone_out_img["backbone_fpn"]  3
        # backbone_out_img["backbone_fpn"][0] shape torch.Size([16, 32, 256, 256])
        # backbone_out_img["vision_features"] shape torch.Size([16, 256, 64, 64])
        # len backbone_out_event["backbone_fpn"]  3
        # backbone_out_event["backbone_fpn"][0] shape torch.Size([16, 32, 256, 256])
        # backbone_out_event["vision_features"] shape torch.Size([16, 256, 64, 64])
        # input.flat_img_batch.shape torch.Size([16, 3, 1024, 1024])
        # processing_order [0]

        # for video input
        # len backbone_out_img["backbone_fpn"]  3
        # backbone_out_img["backbone_fpn"][0] shape torch.Size([8, 32, 256, 256])
        # backbone_out_img["vision_features"] shape torch.Size([8, 256, 64, 64])
        # len backbone_out_event["backbone_fpn"]  3
        # backbone_out_event["backbone_fpn"][0] shape torch.Size([8, 32, 256, 256])
        # backbone_out_event["vision_features"] shape torch.Size([8, 256, 64, 64])
        # input.flat_img_batch.shape torch.Size([8, 3, 1024, 1024])
    ...
    def _prepare_backbone_features_per_frame(self, img_batch, event_batch, img_ids):
        ...
        if inv_ids is not None:
            image = image[inv_ids]
            backbone_fpn = [x[:, inv_ids] for x in backbone_fpn]
            backbone_fpn_evt = [x[:, inv_ids] for x in backbone_fpn_evt]
        ...
        return image, backbone_fpn, backbone_fpn_evt, vision_feats, vision_pos_embeds, vision_feats_event, vision_pos_embeds_event, feat_sizes
            ...
            if video_len != 1:  # have temporal realtion
                ...
                for i in range(self.num_frames_embedding):
                    if stage_id + i + 1 < video_len:
                        ...
                        zero_feat = [torch.zeros_like(x[:, 0]).unsqueeze(1) for x in vision_feats]
                        zero_pos_embed = [torch.zeros_like(x[:, 0]).unsqueeze(1) for x in vision_pos_embeds]
                        zero_feat_event = [torch.zeros_like(x[:, 0]).unsqueeze(1) for x in vision_feats_event]
                        zero_pos_embed_event = [torch.zeros_like(x[:, 0]).unsqueeze(1) for x in vision_pos_embeds_event]
    ...
    def track_step(
        ...
        return current_out, embedding_loss
